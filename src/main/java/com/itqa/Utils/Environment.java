...
public class Environment {
    ...
    public List<Integer> getRandomSilo() {
        switch (getEnv()) {
            case "stg":
                Optional<Integer> optional = Arrays.stream(stg)
                        .boxed()
                        .skip((int) (stg.length * Math.random()))
                        .findAny();
    ...
    public static Boolean getCreateConfluenceSetting() {
        try {
            ...
            shouldUpdateConfluenceStatusOnFailure = Boolean.getBoolean(System.getProperty("updateConfluence").toLowerCase());
        } catch (NullPointerException e) {
            return false;
        }

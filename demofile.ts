function addnumber(a:number,b:number){
    return a+b;
}
let sum:number=addnumber(10,20);
console.log("sum of 2 number:"+sum);


//from starting use "npm install -g typescript"--1 st imstall the type script
//execution command 'tsc Basictypescript_Scripts/day6/array.ts"----2nd after script written you need compile that use this command
// node Basictypescript_Scripts/day6/array.js ------------3rd after complie one js will generate with same name different extension then you execute that js file
//or else use this "node filepath.ts" -----it willnot genrate the one more js file 


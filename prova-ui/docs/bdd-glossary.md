...
| `I press "([^"]*)?"`                                                       | Press a given key. You'll find all supported characters in the [Playwright keyboard reference](https://playwright.dev/docs/api/class-keyboard#keyboard-press). To do that, the value has to correspond to a key from the table. |

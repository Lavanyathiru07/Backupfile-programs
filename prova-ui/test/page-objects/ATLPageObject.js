...
export default class ATLPage {
    ...
    async getHost() {
        let hostName
        let env = process.env.ENV || process.env.appEnv

        if (!env) {
            throw new Error("Neither ENV nor appEnv environment variables are set")
        }

        if (env.includes('http')) {
            if (env.includes('stg01')) {
                env = 'stg01'
            } else if (env.includes('stg02')) {
                env = 'stg02'
            } else if (env.includes('nexus') || env.includes('custjny') || env.includes('posttrvl')) {
                env = 'nexus'
            } else if (env.includes('qa1')) {
                env = 'qa1'
            } else if (env.includes('qa2')) {
                env = 'qa2'
            } else if (env.includes('in1')) {
                env = 'in1'
            } else if (env.includes('in2')) {
                env = 'in2'
            }
        }
        ...
        if (!hostName) {
            throw new Error(`No database host configured for environment: ${env}`)
        }

        console.log("Selected database host:", hostName)
        return hostName
    }
